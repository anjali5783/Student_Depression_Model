...
if model:
    ...
    with st.container():
        ...
        with col2:
            ...
            study_sat = st.select_slider("Satisfaction", options=[1, 2, 3, 4, 5], value=5)
        ...
        with col3:
            ...
            suicidal = st.selectbox("Suicidal Thoughts?", ["No", "Yes"], index=0)
            fin_stress = st.select_slider("Financial Stress", options=[1, 2, 3, 4, 5], value=1)
            fam_history = st.selectbox("Family History", ["No", "Yes"], index=0)
    ...
    with btn_col:
        if st.button("🚀 RUN AI DIAGNOSTIC"):
            if gender == "Select..." or diet == "Select...":
                st.warning("Please select Gender and Diet options.")
            else:
                try:
                    feature_list = [
